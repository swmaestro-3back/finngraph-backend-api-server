package com.finngraph.auth.codegen
...
fun main(args: Array<String>) {
    require(args.size >= 2) { "1개 이상의 스키마 SQL 경로와 출력 디렉터리 파라미터 필요" }
    val outputDir = args.last()
    val schemaFiles = args.dropLast(1).map(Path::of)
    schemaFiles.forEach { require(Files.exists(it)) { "스키마 파일이 없음: $it" } }
...
private fun configuration(postgres: PostgreSQLContainer, outputDir: String) = Configuration()
    ...
    .withGenerator(
        Generator()
            ...
            .withTarget(
                Target()
                    .withPackageName("com.finngraph.auth.adapter.jooq")
